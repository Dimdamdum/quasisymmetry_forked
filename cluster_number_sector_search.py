...
A "sector" here is a tuple (N_0, ..., N_{K-1}) of eigenvalues of the K
...
sub-label, which this module doesn't use). The output is meant to feed a
downstream variational subspace solve (e.g. a matrix-free Lanczos/Davidson
over the union of retained sectors' determinant supports, in the style of
src/selected_sector_lanczos.py) that will improve on the beam search's own
initial low-bond-dimension DMRG energy.
...
Algorithm (see rank_relevant_sectors for the orchestration):
  ...
  B. main_sector_label / gaussian_weight: a cheap, closed-form estimate of
     which sector holds the state's weight, and how much weight
     neighbouring sectors plausibly hold, via a multivariate-Gaussian
     (maximum-entropy, given only the first two RDM-derived moments) model
     of the joint cluster-count distribution.
  ...
  D. coupling_strength_{complex,real}: a PT2-flavoured "how strongly does H
     couple the main sector to this candidate" score, computed directly
     from RDM contractions of h1e/g2e-derived tensors MASKED to the
     specific electron-transfer pattern -- exactly zero, as an exact
     consequence of H having body-rank <= 2, whenever that pattern would
     move 3 or more electrons, which is why only max_elec_transfer<=2
     patterns ever get a direct energy score (deeper candidates fall back
     to the weight channel alone). Two independent implementations are
     provided -- fully general complex, and a real-only fast path (this
     codebase's own h1e/g2e/RDMs are always real in practice, since
     params_to_U_jax only ever emits real orthogonal rotations) -- verified
     to agree with each other and with a from-scratch Fock-space ground
     truth in tests/test_cluster_number_sector_search.py.
  E. sector_dimension: the exact size of a candidate sector's determinant
     support, via a polynomial DP, without ever enumerating it.
...
# =============================================================================
# Step D: coupling-strength q(delta) -- Tier 1 (h1e) / Tier 2 (+g2e/rdm3/rdm4)
#
