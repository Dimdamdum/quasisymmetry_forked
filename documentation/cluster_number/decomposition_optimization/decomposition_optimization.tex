...
\While{true}
  ...
  \For{each $(P,U) \in B$}
    ...
    \For{each of the top $s$ such clusters $C$}
      ...
      \If{\texttt{naturalize\_children}} diagonalize each child's own 1-RDM block \Comment{cost-neutral} \EndIf
...
\paragraph{Local naturalization.} By default, each new child's own 1-RDM block is diagonalized in
place after the block-restricted step -- its orbitals become its own natural orbitals. Exactly
cost-neutral: $N_{V_1}$, $N_{V_2}$ are basis-independent within their own subspace, so no cost
here depends on which orthonormal combination represents a cluster -- disable it via
Section~\ref{sec:usage}'s flag table.
...
\texttt{--no-naturalize-children} & skip the default local naturalization (cost-neutral either way) \\
