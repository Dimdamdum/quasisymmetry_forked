...
(non-commutator) cost against the FCI reference state, for the three
N2 minimal_examples FCIDUMP Hamiltonians.

Local (per-orbital) seniority operators are passed via --parity with
hamiltonians/N2/parity_10_sens.txt, a 10x10 identity matrix — each row
...
Runs optimize_symmetries.py as a subprocess per Hamiltonian, tees its
--verbose per-iteration monitoring output to a log file, and if L-BFGS-B
hasn't converged after --optimizer_maxiter cycles, restarts from the last
rotation (via --x0) for further attempts (up to MAX_ATTEMPTS) so runs
are given every chance to actually converge rather than just stopping at
an iteration cap.
"""
...
MOLPATHS = [
    REPO_ROOT / "hamiltonians/N2/minimal_examples/n2_1_10_ccpvdz_10o10e_FCIDUMP",
    REPO_ROOT / "hamiltonians/N2/minimal_examples/n2_1_50_ccpvdz_10o10e_FCIDUMP",
    REPO_ROOT / "hamiltonians/N2/minimal_examples/n2_2_50_ccpvdz_10o10e_FCIDUMP",
]
PARITY_MATRIX = REPO_ROOT / "hamiltonians/N2/parity_10_sens.txt"  # 10x10 identity -> local seniority per orbital
OUT_DIR = REPO_ROOT / "hamiltonians/N2/minimal_examples/seniority_oo"
